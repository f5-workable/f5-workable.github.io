    ...
    if (targetId === "전체") {
      ...
      if (state.length < 10) {
        ...
        targetChecked
          ...
          : setState(() =>
              state.filter((value) => value !== targetValue || value !== checkboxes[0].value)
            );
      } else {
    ...
    <div className="w-full h-96 flex bg-white rounded-md overflow-hidden border border-solid border-neutral-400">
      ...
      <div className="w-3/4 h-full overflow-y-scroll scrollbar-hide">
        <ul className="grid grid-cols-3">
          ...
          {selectedLocation === "세종" && (
            <>
              <SubRegion value="세종특별자치시" labelValue="전체" onChange={changeLocation} />
            </>
          )}
          {selectedLocation === "경기" && (
            <>
          ...
          {selectedLocation === "제주" && (
            <>
              <SubRegion value="제주특별자치도" labelValue="전체" onChange={changeLocation} />
              <SubRegion
                value="제주특별자치도 서귀포시"
                labelValue="제주 서귀포시"
                onChange={changeLocation}
