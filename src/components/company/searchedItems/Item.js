...
const Item = ({ resume, applicant }) => {
  const [memberInfo, setMemberInfo] = useState();

  const getMemberInfo = useCallback(async () => {
    const info = await api.member.retrieve(applicant?.m_num || resume?.m_num);
    setMemberInfo(info);
  }, [resume, applicant]);
  ...
  return (
    <>
      {applicant ? (
        <Link to={`/resume/applicant/${applicant.cr_num}`} className="block w-full h-80">
          <div className="w-full h-full">
            ...
            <div className="w-full h-2/5 relative">
              ...
              <p className="text-neutral-500">
                {applicant.ob_type} | {applicant.disease}
              </p>
              <p className="py-2 text-neutral-500">{applicant.job}</p>
              <p className="mt-5 font-semibold absolute bottom-2">
                희망{applicant.payment_type} {applicant.payment?.toLocaleString()}원
              </p>
            </div>
          </div>
        </Link>
