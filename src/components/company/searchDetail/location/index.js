...
const Location = ({ state, setState }) => {
  ...
  const changeLocation = (e) => {
    ...
    if (targetId === "전체") {
      ...
      targetChecked
        ? setState(() => [
            ...state.filter((value) => {
              return targetValue.length === 4
                ? value.split(" ")[0] !== targetValue[0] + targetValue[2]
                : value.slice(0, 2) !== targetValue.slice(0, 2);
            }),
            targetValue,
          ])
  ...
  return (
    <div className="w-full h-auto overflow-hidden flex flex-col bg-white rounded-md border border-solid border-neutral-400">
