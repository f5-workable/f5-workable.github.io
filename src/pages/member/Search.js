...
const Search = () => {
  ...
  useEffect(() => {
    if (sortBy === "최신순")
      boards.list?.sort((a, b) => {
        return new Date(b.d_date) - new Date(a.d_date);
      });
  }, [boards, sortBy]);
  ...
  return (
    <main className="w-full h-full py-10">
      <form
        ...
        className="mb-20 w-5/6 max-w-4xl mx-auto bg-neutral-50 p-10 rounded-2xl border-2 border-solid border-neutral-100"
      >
      ...
      {boards.list?.length > 0 && (
        <div className="w-11/12 lg:w-3/4 h-auto mx-auto">
          ...
          <SearchedItems boards={boards.list} />
        </div>
      )}
    </main>
  );
